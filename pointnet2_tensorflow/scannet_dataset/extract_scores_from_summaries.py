...
def load_scores2():
    # PointNet++
    iou = []
    for e in tf.train.summary_iterator('/home/tim/training_log/baseline/working_baseline_250epochs_val/events.out.tfevents.1563534342.tim-desktop'):
        for v in e.summary.value:
            if v.tag == 'iou':
                iou.append(v.simple_value)
    ...
    iou = []
    # Run1
    for e in tf.train.summary_iterator('/home/tim/training_log/pointnet_and_features/long_run1563786310_val/events.out.tfevents.1563786767.tim-desktop'):
        for v in e.summary.value:
            if v.tag == 'iou':
                iou.append(v.simple_value)
    # Run2
    skip_first = 2
    for e in tf.train.summary_iterator('/home/tim/training_log/pointnet_and_features/long_run1563786310_continued_val/events.out.tfevents.1563815651.tim-desktop'):
        for v in e.summary.value:
