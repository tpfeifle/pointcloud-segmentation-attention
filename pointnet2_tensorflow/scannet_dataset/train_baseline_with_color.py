...
N_VAL_SAMPLES = 4542
BATCH_SIZE = 16
LOG_DIR = os.path.join('/home/tim/training_log/baseline/color_test_run_%s' % int(time.time()))
...
def train(epochs=1000, batch_size=BATCH_SIZE, n_epochs_to_val=4):
    ...
    train_features = tf.cast(colors, tf.float32)
    ...
    val_features = tf.cast(colors, tf.float32)
