...
from attention_points.color_scannet import scannet_dataset
...
# Model is changed for compatibility
# MODEL = importlib.import_module(FLAGS.model)  # import network module
# MODEL_FILE = os.path.join(BASE_DIR, FLAGS.model + '.py')

MODEL = importlib.import_module("models.pointnet2_sem_seg_color")
MODEL_FILE = os.path.join(
    '/tmp/pycharm_project_250/pointnet2_tensorflow/color_scannet/models/pointnet2_sem_seg_color.py')
pickle_temp_output = '/tmp/pycharm_project_250/pointnet2_tensorflow/output/batch.pickle'
pickle_temp_output_prediction = '/tmp/pycharm_project_250/pointnet2_tensorflow/output/batch_prediction.pickle'
LOG_DIR = os.path.join('/tmp/pycharm_project_250/pointnet2_tensorflow/log/tim/%s' % int(time.time()))  # FLAGS.log_dir
...
DATA_PATH = '/home/tim/scannet-pre/'
TRAIN_DATASET = scannet_dataset.ScannetDataset(root=DATA_PATH, npoints=NUM_POINT)
...
def generator():
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_color, batch_label, batch_smpw = get_batch_wdp(TRAIN_DATASET, train_idxs, start_idx, end_idx)
        yield batch_data, batch_color, batch_label, batch_smpw


dataset = tf.data.Dataset().from_generator(generator, output_types=(tf.float32, tf.float32, tf.int32, tf.float32))
'''output_shapes=(tf.TensorShape([NUM_POINT, 3]),
              tf.TensorShape([NUM_POINT, 3]),
              tf.TensorShape([NUM_POINT]),
              tf.TensorShape([NUM_POINT])))'''
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            pointclouds_pl, rgb_pl, labels_pl, smpws_pl = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT)
            ...
            # Get model and loss 
            pred, end_points = MODEL.get_model(pointclouds_pl, rgb_pl, is_training_pl, NUM_CLASSES, bn_decay=bn_decay)
...
def get_batch_wdp(dataset, idxs, start_idx, end_idx):
    ...
    for i in range(bsize):
        ps, color, seg, smpw = dataset[idxs[i + start_idx]]
        batch_data[i, ...] = ps
        batch_label[i, :] = seg
        batch_smpw[i, :] = smpw
        batch_color[i, :] = color
    ...
    return batch_data, batch_color, batch_label, batch_smpw
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        end_idx = (batch_idx + 1) * BATCH_SIZE
        # batch_data, batch_color, batch_label, batch_smpw = get_batch_wdp(TRAIN_DATASET, train_idxs, start_idx, end_idx)
        batch_data, batch_color, batch_label, batch_smpw = iterator.get_next()

        # Augment batched point clouds by rotation

        # aug_data = provider.rotate_point_cloud_z(batch_data)
        feed_dict = {ops['pointclouds_pl']: batch_data, # aug_data
                     ops['rgb_pl']: batch_color,
...
def eval_one_epoch(sess, ops, test_writer, epoch_num):
    ...
    for batch_idx in range(int(num_batches)):
        start_idx = batch_idx * BATCH_SIZE
        end_idx = (batch_idx + 1) * BATCH_SIZE
        batch_data, batch_color, batch_label, batch_smpw = get_batch(TEST_DATASET, test_idxs, start_idx, end_idx)
        aug_data = provider.rotate_point_cloud_z(batch_data)
        ...
        with open(pickle_temp_output, 'wb') as fp:
            pickle.dump(batch_data, fp)
            pickle.dump(batch_label, fp)
        with open(pickle_temp_output_prediction, 'wb') as fp:
            pickle.dump(batch_data, fp)
