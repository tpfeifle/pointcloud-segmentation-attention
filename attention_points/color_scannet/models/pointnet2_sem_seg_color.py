...
from attention_points.color_scannet.pointnet_util_color import pointnet_sa_module, pointnet_fp_module


def placeholder_inputs(batch_size, num_point):
    pointclouds_pl = tf.placeholder(tf.float32, shape=(batch_size, num_point, 3))
    rgb_pl = tf.placeholder(tf.float32, shape=(batch_size, num_point, 6))
    labels_pl = tf.placeholder(tf.int32, shape=(batch_size, num_point))
    smpws_pl = tf.placeholder(tf.float32, shape=(batch_size, num_point))
    return pointclouds_pl, rgb_pl, labels_pl, smpws_pl


def get_model(point_cloud, point_cloud_rgb, is_training, num_class, bn_decay=None):
    """ Semantic segmentation PointNet, input is BxNx3 and BxNx6, output Bxnum_class """
    batch_size = point_cloud.get_shape()[0].value
    num_point = point_cloud.get_shape()[1].value
    end_points = {}
    l0_xyz = point_cloud
    l0_points = point_cloud_rgb
...
if __name__ == '__main__':
    with tf.Graph().as_default():
        ...
        colors = tf.zeros((32, 2048, 6))
        net, _ = get_model(inputs, colors, tf.constant(True), 10)
        print(net)
