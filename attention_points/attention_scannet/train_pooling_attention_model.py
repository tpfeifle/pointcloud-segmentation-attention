...
from attention_points.attention_scannet.pooling_attention_model import PoolingAttentionNetModel
...
def get_batch_wdp(dataset, idxs, start_idx, end_idx):  # TIM: wdp?
    ...
    for i in range(bsize):
        ...
        dropout_ratio = np.random.random() * 0.875  # 0-0.875 # TIM: why random?
...
def get_bn_decay(batch):  # TIM: for what?
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            model = PoolingAttentionNetModel(is_training=is_training_pl, bn_decay=None, num_class=NUM_CLASSES)
            ...
            correct = tf.equal(tf.argmax(pred, 2), tf.to_int64(labels_pl))  # TIM: why argmax with 2?
