...
N_VAL_SAMPLES = 4542
BATCH_SIZE = 16
LOG_DIR = os.path.join('/home/tim/training_log/pointnet_and_features/long_run%s' % int(time.time()))
...
def train(epochs=1000, batch_size=BATCH_SIZE, n_epochs_to_val=4):
    ...
    train_features = tf.concat([colors, train_normals], 2)
    print("train features", train_features.shape, train_features)
    ...
    print("val colors", colors.shape)
    print("val normals", normals.shape)
    colors = tf.div(tf.cast(colors, tf.float32), tf.constant(255, dtype=tf.float32))
    val_features = tf.concat([colors, normals], 2)
    print("val_features", val_features.shape, val_features)
    ...
    # train metrics
    train_pred, _ = model.get_model(train_coordinates, train_features, is_training_pl, 21, bn_decay=bn_decay)
