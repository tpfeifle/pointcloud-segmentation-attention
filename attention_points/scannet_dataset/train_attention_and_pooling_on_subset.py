...
import attention_points.models.pointnet2_sem_seg_attention_and_pooling as model
...
BATCH_SIZE = 16
LOG_DIR = os.path.join('/home/tim/training_log/subset/attention_and_pooling%s' % int(time.time()))
...
def train(epochs=300, batch_size=BATCH_SIZE, n_epochs_to_val=4):
    tf.Graph().as_default()
    tf.device('/gpu:0')
