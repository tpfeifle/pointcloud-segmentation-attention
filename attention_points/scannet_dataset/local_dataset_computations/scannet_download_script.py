"""
This is the script provided by scannet with some modifications for multi-threaded download
"""
#!/usr/bin/env python
...
def download_release_in_parallel(release_scans, out_dir, file_types, use_v1_sens):
    ...
    q = Queue()
    threads = []
    print("nr of scans: {}".format(len(release_scans)))
    for e in release_scans:
        q.put(e)
...
def main():
    ...
    if args.task_data:  # download task data
        ...
        download_file(BASE_URL + RELEASE_TASKS + "/" + PREPROCESSED_FRAMES_FILE[0],
        ...
        download_release_in_parallel(release_scans, out_dir_scans, file_types, use_v1_sens=True)
        if not args.v1:
            download_label_map(args.out_dir)
            download_release_in_parallel(release_test_scans, out_dir_test_scans, file_types_test, use_v1_sens=False)
            download_file(BASE_URL + RELEASE_TASKS + "/" + TEST_FRAMES_FILE[0],
                          os.path.join(out_dir_tasks, TEST_FRAMES_FILE[0]))
